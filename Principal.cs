    ...
    public partial class Principal : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<bool> reversedBits = new List<bool>();
                var ReverseBits = ByteToBitArrayReverse(txtArray);
                reversedBits = convertToBitList(ReverseBits);

                //var encodedTxtList = convertToBitList(new BitArray(txtArray));

                 var CipherList = Xor(reversedBits, keyStreamGen(reversedBits.Count, keyAndIVSetup(clave, VectorI)));
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //byte[] bytesTESTFROMHEXA = ConvertHexToBytes(HEXXA);

                BitArray arregloBits = new BitArray(bytesFromHexa);
                var bitReversados = ByteToBitArrayReverse(bytesFromHexa);


                List<bool> listaFinal = new List<bool>();

                listaFinal = convertToBitList(arregloBits);


                var ListaDes = Xor(listaFinal, keyStreamGen(listaFinal.Count, keyAndIVSetup(clave, VectorI)));
                byte[] result = convertToBitArray(ListaDes).ToByteArray();

                //Se aplica un Reverse, ya que cuando se encripta, se aplica el reverse antes del cifrado
                List<bool> reversedBits = new List<bool>();
                var ReverseBits = ByteToBitArrayReverse(result);
                reversedBits = convertToBitList(ReverseBits);

                byte[] Finall = convertToBitArray(reversedBits).ToByteArray();


                rtOutput.Text = System.Text.Encoding.Default.GetString(Finall);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
